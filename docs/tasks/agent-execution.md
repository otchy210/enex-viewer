...
- 進行中タスクのスコープを変えないため、閾値未達の是正を別PRで行う場合は専用タスク（T-021）として切り出してよい。

## 6. タスク完了時のチェック更新
- 各エージェントは、担当タスク完了時に `docs/tasks/implementation-tasks.md` の該当タスク行を `[ ]` から `[x]` へ更新する。
- チェック更新は実装変更と同じ PR に含める。
- `docs/tasks/work-packages/*.md` にはチェックを記録しない（進捗の正本は `implementation-tasks.md` のみ）。
- 担当外タスクのチェックは変更しない。
