...
| [ ]  | T-038 | D      | Web            | 一覧件数セレクター実装（MT-003 改善）                                                            | T-008, T-009, T-020                                                                       | NotesListSection に表示件数のプルダウン（20/50/100 など）を追加し、選択値が `useNotesList` の `limit` に連動して API の `limit` クエリに反映される。入力切替でオフセットがリセットされ、URL 直接編集が不要になる。`manual-test-scenarios.md` MT-003 を Pass できる動作と UI を確認する |
