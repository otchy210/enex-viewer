...
| [ ] | T-032 | J | Refactor | 横断リファクタ（重複解消・共通化・パフォーマンス改善） | T-024, T-031 | ノート一覧の二重取得解消、一覧検索/整形コスト低減、データ変換重複の削減、resource size 算出と multipart 処理の効率改善、非同期 state 実装の統一を行い、既存仕様とテストを維持する |
