...
### レーン P2-A: 永続化 & アップロード基盤（T-201, T-202, T-211）
...
- 受け入れ観点:
  ...
  - 同一ファイルを再アップロードしても既存 importId が返り、UNIQUE 制約違反が発生しない（T-208/T-211）
  - WAL をアップロード毎にチェックポイントし、強制終了後でも DB 側に import が残る
  - テスト実行時（Vitest / `NODE_ENV=test`）は tmp DB 副作用回避のため WAL チェックポイントをスキップする実装方針を維持
