...
| [x]  | T-202 | P2-A   | API     | SQLite 永続化とハッシュ重複判定 (`POST /api/imports/hash-lookup`)              | T-201           | - imports/notes/resources テーブルとユニークハッシュ制約を実装<br>- 同一ファイルを連続アップロードしても再パースせず同じ importId を返す（T-208 で重複 hash 再アップロード時の UNIQUE 制約違反を解消）<br>- API integration test で重複アップロード時にエラーなく importId 再利用を検証     |
