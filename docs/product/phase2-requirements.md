...
- FR-201: API は 2.5 GiB までの multipart ENEX を受け付け、進捗/エラーメッセージを返す。
...
- NFR-201: 2.5 GiB ファイルのアップロード/解析中に UI がフリーズしないよう、進捗表示とキャンセル手段を用意する。
...
- 2.5 GiB テストファイルでアップロード→解析→再起動後の参照が確認できる。
