...
- 手順:
  ...
  2. ページングボタンを往復
- 期待結果:
  - タイトル、日時（`yyyy-MM-dd HH:mm:ss` のローカル時刻）、タグ/抜粋が表示される
...
- 手順:
  1. `small.enex` 取り込み後、既知キーワードで検索して結果が絞り込まれることを確認
  2. 非一致キーワードで検索し 0 件表示になることを確認
  3. 検索入力のクリア操作（入力欄右端の ×、Esc キー、入力値を空文字にする）を行い、一覧が即時に全件表示へ戻ることを確認
  4. クリア直後にページングが先頭ページ（offset=0）へ戻っていることを確認
  5. 一覧上部の `Per page` プルダウンで表示件数を 20 / 50 / 100 に切り替える
  6. 表示件数を切り替えた直後に先頭ページ（1 件目）から再表示されることを確認する
- 期待結果:
  - 一致ノートのみ表示される
  - 非一致では 0 件表示となる
...
- [T-042] 初期スケルトン由来の UI テキスト（TypeScript REST API + Web UI / Hello from ... / 最下部タイムスタンプ）と不要なデモ API 実装を削除する。
- [T-043] 一覧 / 詳細とも ISO 文字列ではなく `yyyy-mm-dd hh:mm:ss` 形式で日時表示する。
- [T-044] 画面上に二つある一覧のうち上部を廃止し、下部 2 ペインの UI に検索機能を統合する。
