...
## MT-217 (Pass): 2.5 GiB クラス ENEX のエンドツーエンド検証（アップロード→閲覧→個別/一括ダウンロード）
...
- 手順
  ...
  2. Web で 2.5 GiB クラス ENEX を選択し、SHA-256 計算完了後に Upload を実行する。
...
- 期待結果
  - 2.5 GiB クラス ENEX でも `ERR_STRING_TOO_LONG` / OOM を出さず import が完了する。
...
2. API を起動し、500MB〜2.5 GiB クラスの ENEX（例: `~/Desktop/ENEX/1100.取扱説明書.enex`）をアップロードして `ERR_STRING_TOO_LONG` や OOM が発生しないことを確認する。
